...
public class ProductService implements Serializable {
	 ...
	 * @return
	 */
	public List<ProdTable> findAllRegisteredProductBD() {
		return productDAO.findProductsByStateBD(RegState.REGISTERED);
	}
	...
	public RetObject validateProduct(ProdApplications prodApplications) {
		...
		try {
			...
			issue= !validateManufacturers(product, issues);
	...
	/**
	 * Validate product manufacturers. Should be FIN or TOLL, but not both!
	 * @param product
	 * @return true, if validation passed, false and list of issues otherwise
	 */
	public boolean validateManufacturers(Product product, List<String> issues){
		if(product != null){
			boolean issue= false;
			String manufName = "";
			List<ProdCompany> prodCompanies = product.getProdCompanies();
			if(prodCompanies == null || prodCompanies.size()<1){
				...
				for(ProdCompany pc : prodCompanies){
					if(pc.getCompanyType().equals(CompanyType.FIN_PROD_MANUF) || pc.getCompanyType().equals(CompanyType.TOLL_MANUF)) {
						finProdManuf++;
						manufName= pc.getCompany().getCompanyName();
					}
				}
				if(finProdManuf == 0) {
				...
				if(finProdManuf != 1){
					issues.add("too_many_fin_prod_manuf");
					issue=true;
				}
			}
			if(!issue){
				product.setManufName(manufName);	
			}
			else{
				product.setManufName("");
			}
			return !issue;
		}else{
			return false;
