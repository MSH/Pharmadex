...
public class CompanyService implements Serializable {
	...
	public List<ProdCompany> addCompany(Product prod, Company selectedCompany, List<String> companyTypes) {
		...
		for (String ct : companyTypes) {
			if(ct != null&&ct != "" && CompanyType.valueOf(ct).equals(CompanyType.FIN_PROD_MANUF)){  //always
				prod.setManufName(selectedCompany.getCompanyName());
			}
			if(ct != null&&ct != "" && CompanyType.valueOf(ct).equals(CompanyType.TOLL_MANUF)){ //only when fin prod does not defined
				if(prod.getManufName()==null || prod.getManufName().length()==0){
					prod.setManufName(selectedCompany.getCompanyName());
				}
			}
