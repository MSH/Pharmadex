...
public class CompanyMBean implements Serializable {
	...
	/**
	 * Check logic, it is impossible to have FIN and TOLL at once
	 * @param companyTypes
	 */
	public void setCompanyTypes(List<String> companyTypes) {
		int manufCount = 0;
		for(String s: companyTypes){
			if (CompanyType.valueOf(s).equals(CompanyType.FIN_PROD_MANUF) ||
					CompanyType.valueOf(s).equals(CompanyType.TOLL_MANUF)){
				manufCount++;
			}
		}
		if(manufCount>1){
			FacesContext fC = FacesContext.getCurrentInstance();
			ResourceBundle rB = facesContext.getApplication().getResourceBundle(fC, "msgs");
			fC.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, rB.getString("too_many_fin_prod_manuf"), ""));
			this.companyTypes = new ArrayList<String>();
		}else{
