	...
	public String onFlowProcess(FlowEvent event) {
		...
		try {
			...
			if(!isClickPrevious(currentWizardStep, nextWizardStep)){
				...
				//check inns! Should be at least one
				if(currentWizardStep.equals("appdetails")){
					...
					if(prodRegAppMbean.getSelectedInns()==null || prodRegAppMbean.getSelectedInns().size()==0){
						context.addMessage(null, 
								new FacesMessage(FacesMessage.SEVERITY_ERROR, resourceBundle.getString("no_inns"), ""));
						nextWizardStep = currentWizardStep;
					}
				}
				//check manufacturers. Should be exactly one Finished or Toll, but not both
				if(currentWizardStep.equals("manufdetail")){
					context = FacesContext.getCurrentInstance();
					ResourceBundle resourceBundle = context.getApplication().getResourceBundle(context, "msgs");
					ProductService prodServ = getProdRegAppMbean().getProductService();
					List<String> issues = new ArrayList<String>();
					if(!prodServ.validateManufacturers(prodRegAppMbean.getProduct(), issues)){
						for (String msg : issues) {
							context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,resourceBundle.getString(msg),""));
						}
						nextWizardStep = currentWizardStep;
					}
				}
